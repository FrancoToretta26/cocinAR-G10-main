...
export default class EditarReceta extends Component {
    ...
    pickImage = async () => {
      ...
      async componentDidMount(){ 
        ...
        console.log(this.state.porciones);
        await this.setState({porciones: parseInt( params.receta.porciones)})
        console.log(this.state.porciones);
        
        
        await this.setState({ ingredientes: this.state.ingredientes.map((c, index) => index != -1 ? { ...c, 
            nombre: params.ingredienteConCantidad[index].nombre, 
            cantidad: String(params.ingredienteConCantidad[index].cantidad), 
            medida: params.ingredienteConCantidad[index].medida } : c) 
        })
        await this.setState({ pasos: this.state.pasos.map((c, index) => index != -1 ? { ...c, 
            descripcion: params.pasos[index].descripcion, 
            multimedia: params.pasos[index].multimedia,  } : c, console.log(c)) 
        })
        this.setState({categoria: params.receta.tagString})
      };
